...
function readRuntimeCookieFile() {
  try {
    if (!fs.existsSync(RUNTIME_COOKIE_PATH)) {
      return null;
    }
    const parsed = JSON.parse(fs.readFileSync(RUNTIME_COOKIE_PATH, "utf8"));
...
function getCookieFromEnv() {
  const runtimeCookie = readRuntimeCookieFile();
  if (runtimeCookie?.cookie) {
    return runtimeCookie.cookie;
  }
...
function getCookieDiagnostics() {
  const runtimeCookie = readRuntimeCookieFile();
  let fileCookie = "";
  let envFileExists = false;
  let envFileMtime = null;

  try {
    if (fs.existsSync(ENV_PATH)) {
      envFileExists = true;
      const stats = fs.statSync(ENV_PATH);
      envFileMtime = new Date(stats.mtimeMs).toISOString();
      const env = dotenv.parse(fs.readFileSync(ENV_PATH, "utf8"));
      fileCookie = env.MARKETECH_COOKIE || "";
    }
  } catch {
    // Best-effort diagnostics only.
  }

  const processCookie = process.env.MARKETECH_COOKIE || "";
  const runtimeCookieValue = runtimeCookie?.cookie || "";
  const activeCookie = runtimeCookieValue || fileCookie || processCookie || "";

  return {
    runtimeCookieExists: Boolean(runtimeCookie),
    runtimeCookieUpdatedAt: runtimeCookie?.updatedAt || null,
    runtimeCookiePresent: Boolean(runtimeCookieValue),
    runtimeCookieLength: runtimeCookieValue.length,
    runtimeCookieHash: hashCookieValue(runtimeCookieValue),
    envFileExists,
    envFileMtime,
    activeCookieSource: runtimeCookieValue ? "runtime_file" : fileCookie ? "env_file" : processCookie ? "process_env" : "missing",
    fileCookiePresent: Boolean(fileCookie),
    processCookiePresent: Boolean(processCookie),
    activeCookiePresent: Boolean(activeCookie),
    fileCookieLength: fileCookie.length,
    processCookieLength: processCookie.length,
    activeCookieLength: activeCookie.length,
    fileCookieHash: hashCookieValue(fileCookie),
    processCookieHash: hashCookieValue(processCookie),
    activeCookieHash: hashCookieValue(activeCookie),
    fileMatchesProcessEnv: Boolean(fileCookie) && Boolean(processCookie) && fileCookie === processCookie
  };
}
...
function getChartRangeConfig(intervalKey) {
  ...
  if (normalized === "day") {
    return {
      ...
      startOffsetMs: 91 * 24 * 60 * 60 * 1000,
      endSnap: "day"
    };
  }
  ...
  if (normalized === "month") {
    return {
      ...
      startOffsetMs: 3 * 365 * 24 * 60 * 60 * 1000,
      endSnap: "month"
    };
  }
...
async function searchContracts() {
  const payload = await postJson("/contracts/search", {
    searchTerm: {
      Terms: [
        {
          Text: DEFAULT_SEARCH_PRODUCT_TYPE,
          Fields: "ProductType"
        }
      ]
    }
  ...
  return raw
    .filter((item) =>
      item &&
      item.Exchange === "XASX" &&
      item.ProductType === DEFAULT_SEARCH_PRODUCT_TYPE &&
      isValidSymbol(item.Symbol)
    )
    .map((item) => ({
      symbol: normalizeSymbol(item.Symbol),
      name: item.Name || null,
      productType: item.ProductType || DEFAULT_SEARCH_PRODUCT_TYPE,
